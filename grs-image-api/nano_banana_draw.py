...
"""
Nano Banana 作图工具 - 基于 GrsAI API 封装
专门使用 nano-banana-fast 模型进行高速图片生成

API 地址: POST https://grsaiapi.com/v1/api/generate
结果轮询: GET https://grsaiapi.com/v1/api/result?id=<task_id>

注意：生成的四宫格图片需使用 四宫格切分.py 切分，本模型参数为 -p 25 --min-gap 25
"""
...
RESULT_URL = "https://grsaiapi.com/v1/api/result"
...
def submit_draw_task(prompt: str, aspect_ratio: str = "3:2", base64_images: list = None, image_size: str = "1K", timeout: int = 600) -> str:
    """提交 nano-banana-fast 绘图任务，返回 task_id"""
    payload = {
        "model": "nano-banana-fast",
        "prompt": prompt,
        "aspectRatio": normalize_aspect_ratio(aspect_ratio),
        "images": base64_images if base64_images else [],
...
def poll_draw_result(task_id: str, output_path: str, poll_interval: int = 3, timeout_sec: int = 1200):
    ...
    while True:
        ...
        print(f"⏳ [nano-banana-fast] 任务状态: {status} (进度: {progress}%)")
...
def draw(prompt: str, output_path: str, aspect_ratio: str = "3:2", reference_images: list = None, image_size: str = "1K"):
    """一站式 nano-banana-fast 作图函数"""
    base64_images = []
    if reference_images:
    ...
    print(f"\n🎨 [nano-banana-fast] 正在发送图像生成请求 -> 目标文件: {os.path.basename(output_path)}")
    task_id = submit_draw_task(prompt, aspect_ratio, base64_images, image_size)
    if not task_id:
...
def main():
    parser = argparse.ArgumentParser(description="nano-banana-fast 专有 API 作图脚本")
    parser.add_argument("--prompt", type=str, required=True, help="作图提示词")
    parser.add_argument("--output", type=str, required=True, help="图片保存路径")
    parser.add_argument("--aspect_ratio", "--ratio", "--resolution", type=str, default="3:2",
                        help="图片比例，如 1:1, 16:9, 9:16, 4:3, 3:4, 3:2, 2:3, 5:4, 4:5, 21:9")
    parser.add_argument("--image_size", type=str, default="1K", choices=["1K", "2K", "4K"], help="分辨率尺寸")
    parser.add_argument("--image1", type=str, help="参考图 1 路径")
    parser.add_argument("--image2", type=str, help="参考图 2 路径")
    parser.add_argument("--image3", type=str, help="参考图 3 路径")
    parser.add_argument("--image4", type=str, help="参考图 4 路径")
    ...
    draw(args.prompt, args.output, args.aspect_ratio, image_paths if image_paths else None, args.image_size)
