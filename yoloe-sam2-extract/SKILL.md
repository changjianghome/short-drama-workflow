...
本工作区有**两个**卡通角色提取 Skill，效果相近，区别在用法：

| | **本 Skill (YOLOE+SAM2.1)** | **image-person-extraction** |
|:---|:---|:---|
| 体积 | 1.4GB | 3.2GB |
| 用法 | 文字提示**自动找所有**卡通角色，批量出 | 文字提示**指定目标**，逐一提（"提取那只狐狸"） |
| 适用 | 要**一页全部**角色 | 要抠**特定某个/几个**角色 |
| 部署 | 需下载3模型+建软链，较复杂 | `from_pretrained` 自动下载，简单 |

**用户说"提取所有卡通人物"（全页批量）→ 用本 Skill（YOLOE+SAM2.1）**；
**用户说"提取图中的XX"（指定目标）→ 用 image-person-extraction（SAM3）**。
