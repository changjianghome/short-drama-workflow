...
| | **本 Skill (SAM3)** | **yoloe-sam2-extract** |
|:---|:---|:---|
| 体积 | 3.2GB | 1.4GB |
| 用法 | 文字提示**指定目标**，逐一提（"提取那只狐狸"） | 文字提示**自动找所有**卡通角色，批量出 |
| 适用 | 要抠**特定某个/几个**角色 | 要**一页全部**角色 |
| 部署 | `from_pretrained` 自动下载，简单 | 需下载3模型+建软链，较复杂 |

**用户说"提取图中的XX"（指定目标）→ 用本 Skill（SAM3）**；
**用户说"提取所有卡通人物"（全页批量）→ 用 yoloe-sam2-extract**。
